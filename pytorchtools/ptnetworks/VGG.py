...
class VGG(nn.Module):
    def __init__(self,
        ...
        if variant == 'vgg11_bn':
            self.embedded_model = vgg11_bn(**arg_dict)
        elif variant == 'vgg13_bn':
            self.embedded_model = vgg13_bn(**arg_dict)
        elif variant == 'vgg16_bn':
            self.embedded_model = vgg16_bn(**arg_dict)
        elif variant == 'vgg19_bn':
            self.embedded_model = vgg19_bn(**arg_dict)
        else:
...
def _vgg(arch: str, cfg: str, batch_norm: bool, pretrained: bool, progress: bool, **kwargs: Any) -> VGG:
    ...
    model = VGG_(make_layers(cfgs[cfg], batch_norm=batch_norm), **kwargs)
    if pretrained:
